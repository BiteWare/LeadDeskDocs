...
<Steps>
  <Step title="Receive Job">
    Webhook receives job_id and practice_count from backend
  </Step>
  
  <Step title="Update Job Status">
    Set job status to `processing` in Supabase
  </Step>
  
  <Step title="Fetch Practices">
    Query all practices for the job from `job_practices` table
  </Step>
  
  <Step title="Create Batches">
    Chunk practices into groups of 10-25 for parallel processing
  </Step>
  
  <Step title="Trigger URL Worker">
    Call URL Worker webhook for each batch
  </Step>
</Steps>
...
<Card title="URL Worker" icon="link" href="/workflows/url-worker">
  Configure the URL normalization worker
</Card>

<Card title="Monitor Jobs" icon="chart-line" href="/architecture/database-schema">
