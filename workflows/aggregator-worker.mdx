...
<Steps>
  <Step title="Fetch Practice Data">
    Query Supabase for complete practice record
    - URL worker payload
  ...
  <Step title="Update Database">
    Write final payload to Supabase
    - Store `final_payload`
...
1. When practice status → `aggregated`
2. Increment `completed_practices` count
3. If `completed_practices` == `total_practices`
...
- **Processing time**: <100ms per practice
...
<Card title="Export Results" icon="download" href="/api-reference/endpoints/export-job">
...
<Card title="Quality Metrics" icon="chart-line" href="/workflows/overview">
